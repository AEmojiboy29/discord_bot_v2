...
async def whitelist_command(ctx, action: str, user_id: int = None):
    ...
    if action.lower() == "add":
        ...
        if api_add_whitelist(user_id, "Manual_Add", ctx.author.name):
            ...
            embed.add_field(name="API Check", value=f"`{WEB_API_URL}/check_whitelist?user_id={user_id}`", inline=False)
        ...
        try:
            ...
            if data and data.get('status') == 'success':
                ...
                if whitelist_data:
                    ...
                    embed.set_footer(text=f"Total: {len(whitelist_data)} users - See full list at {WEB_API_URL}/whitelist")
                    await ctx.send(embed=embed)
                else:
                    embed = discord.Embed(
                        title="📋 Whitelisted Users",
        ...
        embed.add_field(
            name="Add User",
            value=f"`POST {WEB_API_URL}/whitelist/add`",
            inline=False
        )
        embed.add_field(
            name="Remove User",
            value=f"`POST {WEB_API_URL}/whitelist/remove`\n`DELETE {WEB_API_URL}/whitelist/USERID`",
            inline=False
        )
        embed.add_field(
            name="Server Status",
            value=f"`{WEB_API_URL}/status`",
            inline=False
        )
        await ctx.send(embed=embed)
